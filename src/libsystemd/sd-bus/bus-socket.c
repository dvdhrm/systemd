...
static int bus_socket_start_auth_client(sd_bus *b) {
        static char sasl_auth_anonymous[] = {
                /*
        ...
        static char sasl_auth_external[] = {
                "\0AUTH EXTERNAL\r\n"
                "DATA\r\n"
        };
        static char sasl_negotiate_unix_fd[] = {
                "NEGOTIATE_UNIX_FD\r\n"
        };
        static char sasl_begin[] = {
                "BEGIN\r\n"
        };
        ...
        if (b->anonymous_auth)
                b->auth_iovec[0] = IOVEC_MAKE(sasl_auth_anonymous,
                                              1 + strlen(sasl_auth_anonymous + 1));
        else
                b->auth_iovec[0] = IOVEC_MAKE(sasl_auth_external,
                                              1 + strlen(sasl_auth_external + 1));

        if (b->accept_fd)
                b->auth_iovec[1] = IOVEC_MAKE_STRING(sasl_negotiate_unix_fd);
        else
                b->auth_iovec[1] = IOVEC_MAKE(NULL, 0);

        b->auth_iovec[2] = IOVEC_MAKE_STRING(sasl_begin);
